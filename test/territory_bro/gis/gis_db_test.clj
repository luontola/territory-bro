...
(defn- gis-db-spec [username password schema]
  (let [url (str/replace-first (:database-url config/env) "jdbc:" "")
        url (str "jdbc:" (-> (URIBuilder. url)
                             (.setParameter "user" username)
                             (.setParameter "password" password)
                             (.setParameter "currentSchema" (str schema ",public"))))]
    {:connection-uri url}))

(defn- login-as [username password]
  (with-open [conn (jdbc/get-connection (gis-db-spec username password test-schema))]
...
(deftest gis-user-database-access-test
  (let [password "password123"
        user-db-spec (gis-db-spec test-username password test-schema)]
