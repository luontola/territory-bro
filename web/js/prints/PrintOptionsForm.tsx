      ...
      return <>
              ...
              <fieldset>
                ...
                <div className="pure-g">
                  <div className="pure-u-1 pure-u-md-1-2 pure-u-lg-1-3">
                    ...
                    <Field name="mapRaster" id="mapRaster" component="select" className="pure-input-1">
                      {availableMapRasters.map(mapRaster => <option key={mapRaster.id} value={mapRaster.id}>{mapRaster.name}</option>)}
                    </Field>
                  </div>
                </div>
                ...
                {template.type === 'region' && <div className="pure-g">
                  <div className="pure-u-1 pure-u-md-1-2 pure-u-lg-1-3">
                    ...
                    <Field name="regions" id="regions" component="select" multiple size={7} className="pure-input-1" onChange={event => setFieldValue("regions", [].slice.call(event.target.selectedOptions).map(option => option.value))}>
                ...
                {template.type === 'territory' && <div className="pure-g">
                  <div className="pure-u-1 pure-u-md-1-2 pure-u-lg-1-3">
                    ...
                    <Field name="territories" id="territories" component="select" multiple size={7} className="pure-input-1" onChange={event => setFieldValue("territories", [].slice.call(event.target.selectedOptions).map(option => option.value))}>
          ...
          <IntlProvider locale={values.language} messages={getMessages(values.language)}>
            <div lang={values.language}>
              ...
              return <template.component key={territoryId} territoryId={territoryId} congregationId={congregationId} mapRaster={mapRaster} />;
            })}
              {template.type === 'region' && values.regions.map(regionId => {
              return <template.component key={regionId} regionId={regionId} congregationId={congregationId} mapRaster={mapRaster} />;
            })}
            </div>
          </IntlProvider>
        </>;
