...
function QgisProjectSection({
  ...
  return <>
      ...
      <p>For more help, read the guides at <a href="https://territorybro.com/guide/">https://territorybro.com/guide/</a> or ask <a href={`mailto:${supportEmail}`}>{supportEmail}</a>.
      </p>
    </>;
}
...
const OverviewPage = () => {
  ...
  return <>
      ...
      {congregations.length > 0 && <>
        ...
        <ul>
          {congregations.map(cong => <li key={cong.id} style={{ fontSize: '150%' }}><Link to={`/congregation/${cong.id}`}>{cong.name}</Link></li>)}
        </ul>
